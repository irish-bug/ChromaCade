...
- `boot_chime.sh` / `chromacade-boot-chime.service` — the boot chime, a separate systemd unit from `chromacade.service` (repo root). Cycles through `prompts/hello.wav`/`whats_up.wav`/`lets_go.wav` (added 2026-08-27), not a single fixed clip.
