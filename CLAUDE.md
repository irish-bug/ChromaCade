...
The two pieces fasten together along X (the case's width axis), right at each piece's own edge — not across the p1/p5 seam (an earlier attempt put mounting bosses there, needing screws up to 6" long, since a piece's own wall material is cut back well short of the seam corner at any real distance from the exact edge). This needs no added boss geometry: each piece's strips/endcap are already continuous, print-safe solid material, so a bore straight into that existing material gives a short screw path directly. See `enclosure/case-design.md`'s "Fastening" paragraph for the full 9-mount layout (4 on pot-side, 5 on blank-side) before changing any mount position.
