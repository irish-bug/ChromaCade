...
# Render a specific file (e.g. test plates, the bottom+back bracket) to check syntax/geometry
openscad -o /tmp/out.stl enclosure/chromacade-bottom-back.scad
...
The case is an **arcade-cabinet profile**, split into two printed pieces as of 2026-08-17 (see `docs/decision-log.md`): a main shell (sides, front wall, shelf, panel) and a removable bottom+back L-bracket, screwed together. A 2D cross-section (front-to-back) is extruded across the case width, then hollowed out and cut with hardware openings. Each `.scad` file (all under `enclosure/`) is independently renderable (not `include`d by one another) — they share the same dimension constants by copy-paste, which is a known deliberate tradeoff, not an oversight (see git history: "Completely unlinked standalone files", "Revert to monolithic file with export toggles"). **When changing a shared dimension (`case_w`, `case_d`, `wall`, `front_h`, `shelf_d`, `shelf_a`, `panel_l`, `panel_a`) or a boss position (`boss_x`, `boss_z_top`, `boss_z_bot`, `boss_z_top_ctr`, `boss_z_bot_ctr`, `boss_y_front`, `boss_y_front_ctr`), update it in every file that redeclares it** — currently `enclosure/chromacade-housing.scad`, `enclosure/chromacade-housing-embossed.scad`, and `enclosure/chromacade-bottom-back.scad` all carry the full block; the boss positions specifically must also stay in sync between the shell files (which place the bosses) and the bracket file (which places matching screw clearance holes) or the two pieces won't line up.

Files (all under `enclosure/`):
- `chromacade-housing.scad` — the main shell: builds the 2D profile (`p0`..`p5`) from the dimension constants, extrudes it across `case_w`, shells it by `wall` thickness, cuts the bottom and back openings (`bottom_hole()`/`back_hole()`), adds 9 mounting bosses for the bottom+back bracket to screw into, and cuts all hardware openings (speaker grilles, shelf controls, panel controls, side-panel hole) via `hardware_cutouts()`. Kept around only until the embossed variant's wordmark print is validated on real hardware — `chromacade-housing-embossed.scad` below is the current, actively-developed model.
- `chromacade-housing-embossed.scad` — **the current primary shell model.** Wordmark variant of the main shell: identical geometry plus a raised "ChromaCade" emboss on the panel exterior between the OLED and LED ring. Kept as a separate standalone file rather than a toggle, same rationale as the rest of this file split (see `docs/decision-log.md`). The letter outlines are embedded as literal polygon data (converted from `ChromaCade-wordmark-paths.svg`, kept in-repo as the source asset) rather than `import()`-ed at render time, so this file needs no external file access or fonts to render — an earlier `text()`-based emboss needed the Comfortaa font, which OpenSCAD couldn't reliably reproduce, hence the switch to traced polygon paths.
- `chromacade-bottom-back.scad` — the removable bottom+back L-bracket (replaces the old `chromacade-back-panel.scad`, retired 2026-08-17, which covered only the back and left the base fused to the shell). Two flat wall-thick plates — a bottom leg spanning `case_d` and a back leg spanning `case_h` — meeting at a 90° corner with a small fillet (see `bottom_back_profile()`; the profile is built directly in the (depth,height) plane and mapped into the shell's (X,Y,Z) frame via `rotate(a=120, v=[1,1,1])`, the cyclic-permutation rotation that avoids a sign-flipped axis — see that module's comment before changing it). Carries the power-cable passthrough and the unpopulated fan-vent cutout, both repositioned unchanged from the old back-panel file.
