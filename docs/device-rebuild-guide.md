...
**`Environment=CHROMACADE_OLED_ROTATION` is also per-device, added 2026-08-28** — see `oled_display.py`'s own comment for the full story: chromacade and plinkplonk's OLED panels are physically mounted 180 degrees opposite each other, so a single hardcoded rotation value in the code can never be right for both, and merging one device's fix into the shared branch silently flips the other (confirmed live: merging `plinkplonk` into `main` and pulling to `chromacade` flipped its display upside down). `oled_display.py` now reads this from the environment, defaulting to `180` (plinkplonk's orientation) if unset. The tracked `chromacade.service` sets it to `180` explicitly for that reason. **If installing on `chromacade` instead, change this line to `Environment=CHROMACADE_OLED_ROTATION=0`** before copying the file — do this same check on any *new* unit too, don't assume either value is correct without actually looking at the display after enabling.
