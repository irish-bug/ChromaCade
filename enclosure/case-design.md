...
See `chromacade-pot-side.scad`'s `pot_side_mounts()`/`pot_side_mount_bosses()`/`pot_side_clearance_holes()` and the matching set in `chromacade-blank-side.scad`. **`chromacade-blank-side-embossed.scad` has not been updated to this mount system yet** (still has the earlier bare-hole, 3mm/3.5mm design) — a rename is under consideration, so it was deliberately left alone; update it to match `chromacade-blank-side.scad` before treating it as current again.
