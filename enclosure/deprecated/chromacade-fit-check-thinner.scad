...
// (THINNER PAIR: syncs chromacade-blank-side-thinner.scad +
//  chromacade-pot-side-thinner.scad, both as of 2026-08-26)
//
// NEW FILE 2026-08-26. chromacade-fit-check.scad tracks the ORIGINAL pair
// (wall=5 both endcaps, joystick posts) and is left alone; this tracks the
// -thinner pair actually being printed. Synced differences that affect fit:
//   - wall_thin=3 endcaps on BOTH pieces, via endcap_relief() applied to
//     each piece's endcap+strips TOGETHER (see that module's bug note)
//   - blank-side's joystick posts replaced by two ramped ribs
//   - blank-side's joystick at shelf-local joy_y_offset, NOT y=0. The
//     original fit-check still places the stick hole, gimbal sphere and
//     boss cluster at y=0, which went stale when shelf_d became 60 on
//     2026-08-22 -- it has been drawing the joystick 10mm off ever since.
//   - KY-023 board outline MEASURED 2026-08-26 (was ESTIMATE-class)
//   - pot-side's mount + Pi bosses anchored at wall_thin
//
...
wall   = 5;
// Relieved endcap thickness, both pieces. Mount bosses stay full size
// regardless -- see either -thinner file's wall_thin comment.
wall_thin = 3;
...
// Joystick Y on the shelf: 20mm reserved at the front, stick centered in
// what remains -> -10 for shelf_d=60. The original fit-check omits this
// and draws the joystick at y=0; that 10mm error is the main reason this
// file exists as its own copy.
joy_reserved_front = 20;
joy_y_offset = shelf_d/2 - (joy_reserved_front + (shelf_d - joy_reserved_front)/2);
...
// Re-MEASURED 2026-08-26, superseding the 2026-08-18 figures (26x33):
// 27mm wide; the edge nearest the speaker is 2.5mm beyond the front hole
// centers; 34mm from that edge to the back; header another 5mm past it.
// JOY_PCB_FRONT_DY is what makes the board land right relative to the
// holes -- the old mockup just centered the board on the hole cluster.
JOY_PCB_W       = 27;   // MEASURED 2026-08-26
JOY_PCB_H       = 34;   // MEASURED 2026-08-26
JOY_PCB_FRONT_DY = 2.5; // MEASURED 2026-08-26: front edge beyond front holes
...
// Shaves the (wall-wall_thin) outermost slice off one piece's X extreme.
// MUST be applied to that piece's endcap AND strips together: strips()
// reaches the same true exterior face, so relieving the endcap alone
// leaves the wall strips standing 2mm proud, which reads from outside as a
// triangular/wedge bite out of the endcap (recess bounded by the diagonal
// p1-p5 seam). Found and fixed on blank-side, then found again on
// pot-side, both 2026-08-26.
...
// 2D ring between the wall_thin and wall insets -- pot-side's INTERIOR
// strip relief. Different job from endcap_relief(): thins the strips' wall
// thickness in the profile plane, does not touch their X extent. Both are
// applied; neither substitutes for the other.
...
// All four anchored at wall_thin, matching the -thinner file: a boss left
// at wall would float off the now-shallower strip material.
module pot_side_mount_bosses() {
    square_boss(pot_edge_x, 110, "z", z_start=wall_thin, height=boss_w);
    square_boss(pot_edge_x, 30, "z", z_start=wall_thin, height=boss_w);
    square_boss(pot_edge_x, 85, "y", y_start=wall_thin, height=boss_w);
    square_boss(pot_edge_x, 20, "y", y_start=wall_thin, height=boss_w);
}

module pot_side_mounts() {
    translate([pot_edge_x, 110, wall_thin + boss_w/2]) rotate([0, -90, 0]) cylinder(h=engage, d=pilot_d);
    translate([pot_edge_x, 30, wall_thin + boss_w/2])  rotate([0, -90, 0]) cylinder(h=engage, d=pilot_d);
    translate([pot_edge_x, wall_thin + boss_w/2, 85])  rotate([0, -90, 0]) cylinder(h=engage, d=pilot_d);
    translate([pot_edge_x, wall_thin + boss_w/2, 20])  rotate([0, -90, 0]) cylinder(h=engage, d=pilot_d);
}
    ...
    difference() {
        union() {
            difference() {
                union() {
                    endcap(-1);
                    difference() { pot_strips(); pot_strips_relief(); }
                }
                endcap_relief(-1);
            }
...
module flat_mount_boss(pos, thick_axis, wall_z=-wall, wall_y=-wall, w=boss_w, body=boss_body, ramp=boss_ramp) {
    if (thick_axis == "z") {
        translate([blank_edge_x, pos - w/2, wall_z - w])
        cube([body, w, w]);

        hull() {
            translate([blank_edge_x + body - 0.01, pos - w/2, wall_z - w])
            cube([0.01, w, w]);

            translate([blank_edge_x + body + ramp - 0.01, pos - w/2, wall_z - 0.01])
            cube([0.01, w, 0.01]);
        }
    } else {
        translate([blank_edge_x, wall_y - w, pos - w/2])
        cube([body, w, w]);

        hull() {
            translate([blank_edge_x + body - 0.01, wall_y - w, pos - w/2])
            cube([0.01, w, w]);

            translate([blank_edge_x + body + ramp - 0.01, wall_y - 0.01, pos - w/2])
            cube([0.01, 0.01, w]);
        }
    }
}
...
module blank_side_mount_bosses() {
    // front_bottom: smaller 8mm cross-section (speaker layout), taper kept.
...
module blank_side_mounts() {
    // front_bottom's bore is shallower: its boss is the small 8mm variant,
    // so the shared engage=15 would run out into the taper's thin end.
    translate([blank_edge_x, own_mount_boss_centers[0][0], own_mount_boss_centers[0][1]])
    rotate([0, 90, 0]) cylinder(h=11, d=pilot_d);

    for (i = [1:3]) {
        translate([blank_edge_x, own_mount_boss_centers[i][0], own_mount_boss_centers[i][1]])
        rotate([0, 90, 0]) cylinder(h=engage, d=pilot_d);
    }
}

// Gap-blocking bridges ("gap lips") -- REMOVED 2026-08-22, mirroring
// their removal from chromacade-blank-side.scad. Confirmed via direct
// visual inspection that they didn't actually close the window where
// the two pieces don't meet along X at the bottom-front/top-back --
// see that file's comment for the full (now-obsolete) design history.
...
module blank_hardware_cutouts() {
    ...
    rotate([-shelf_a, 0, 0]) {
        ...
        // At joy_y_offset, NOT y=0: see this file's header.
        translate([-65, joy_y_offset, 0]) cylinder(h=wall*4, d=JOY_STICK_D, center=true);
...
// TWO RAMPED RIBS, replacing the four posts (2026-08-26). The posts were
// four bare cantilevers whose first layer began in mid-air at ~162.8mm of
// print height -- four support towers, ~6h and most of the wasted PLA on
// this piece. Ribs run along X (a rib spanning Y would face the build axis
// and be one large overhang) and each grows out of the shelf via a +X
// ramp: 45deg for 9mm, then a 3mm vertical step that bridges. The step
// lands at X=-44, clearing the front encoder's body edge at -42.5 by
// 1.5mm -- do not lengthen joy_rib_ramp without rechecking that.
joy_rib_dx       = joy_hole_dx + 3;          // +-12
joy_rib_front_y0 = joy_hole_front_dy - 5;    // +9
joy_rib_front_y1 = joy_hole_front_dy + 3;    // +17  <- speaker-limited
...
module joystick_mockup() {
    ...
    translate([0, shelf_my, shelf_mz])
    rotate([-shelf_a, 0, 0])
    translate([0, joy_y_offset, 0]) {
        // board's near (top) face rests on the rib tips
        board_z = -wall - joy_boss_h;
        // Located off the MEASURED front edge, not centered on the holes:
        // front edge = joy_hole_front_dy + JOY_PCB_FRONT_DY = +16.5, which
        // sits just BEHIND the front rib's +17 face -- so the rib, not the
        // board, stays the part controlling speaker clearance.
        pcb_y1 = joy_hole_front_dy + JOY_PCB_FRONT_DY;
        pcb_y0 = pcb_y1 - JOY_PCB_H;

        translate([joy_x - JOY_PCB_W/2, pcb_y0, board_z - JOY_PCB_T])
            cube([JOY_PCB_W, JOY_PCB_H, JOY_PCB_T]);

        // header off the board's back edge, reaching to -22.5. That is 2.5mm
        // past the shelf's own rear edge, which is NOT a collision: it sits
        // 12mm below the shelf and the panel rises away from p3 at 45deg,
        // leaving ~10mm to the panel's interior face.
        translate([joy_x - 6.5, pcb_y0 - JOY_HEADER_LEN, board_z - JOY_PCB_T])
            cube([13, JOY_HEADER_LEN, JOY_PCB_T]);
