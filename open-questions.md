...
- **Physical button color vs. ring color mapping:** the sourced rainbow keycaps (Elacgap, 8 usable colors) are "distinct but not strictly ROYGBIV" — need to finalize which physical cap color maps to which letter, ideally matching the compressed hue-arc scheme once that's settled.
